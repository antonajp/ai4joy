...
class AudioSession:
    ...
    Attributes:
        ...
        turn_manager: Turn manager for turn counting
...
class AudioStreamOrchestrator:
    ...
    Note: Each session gets its own MC agent instance for isolation.
    """
    ...
    def _create_runner_for_session(self, session: AudioSession) -> Any:
        """Create a Runner for a specific session with its MC agent.

        Creates a new Runner instance for each session to ensure proper
        agent isolation.

        Args:
            session: AudioSession with the MC agent to use
    ...
    async def start_session(
        ...
        # Initialize turn manager for turn counting
        turn_manager = AgentTurnManager(starting_turn_count=starting_turn_count)

        # Create per-session MC agent to ensure isolation between concurrent sessions
    ...
    async def stream_responses(
        ...
        """Stream responses from the MC agent.

        Args:
            session_id: Session identifier

        Yields:
            Response dicts with audio, transcription, or error messages
        """
        session = self._sessions.get(session_id)
        if not session:
        ...
        # Create per-session runner with the session's MC agent
    ...
    async def _process_event(self, event: Any, session: AudioSession) -> list[Dict[str, Any]]:
        ...
        Args:
            ...
            session: Audio session for context
