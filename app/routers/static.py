...
async def serve_static_file(file_path: str, request: Request):
    ...
    try:
        ...
        logger.debug("Serving static file", path=file_path, media_type=media_type)

        return FileResponse(path=file_full_path, media_type=media_type)

    except Exception as e:
        logger.error("Error serving static file", path=file_path, error=str(e))
