...
class TestRealADKExecution:
    ...
    async def test_session(self, session_manager):
        ...
        session_data = SessionCreate(
            location="Mars Colony Research Station",
    ...
    @pytest.mark.asyncio
    async def test_tc_real_adk_02_phase_transition(
        self, orchestrator, test_session
    ):
        """
        TC-REAL-ADK-02: Phase Transition at Turn 5
        ...
        fallibility_indicators = [
            'wait', 'but', 'error', 'problem', 'confused',
            'uncertain', 'not sure', 'maybe', 'help'
        ]
    ...
    async def test_tc_real_adk_03_coach_feedback_at_turn_15(
        ...
        """
        user_inputs = [
            f"This is improv line {i} to build up our scene."
            for i in range(1, 16)
        ]
    ...
    @pytest.mark.asyncio
    async def test_tc_real_adk_05_timeout_handling(
        self, orchestrator
    ):
        """
        TC-REAL-ADK-05: Timeout Handling
