...
class ScenarioLoader:
    ...
    def get_scenarios_by_category(self, category: str) -> List[Dict[str, Any]]:
        ...
        return [s for s in self.scenarios if category.lower() in s.get('name', '').lower()]
...
class ResponseValidator:
    ...
    def validate_response(  # noqa: C901
        ...
        if required_keywords and keywords_found < min_keywords_required:
            errors.append(f"Insufficient keywords: found {keywords_found}/{len(required_keywords)}, need {min_keywords_required}")
...
class EvaluationRunner:
    ...
    async def run_single_evaluation(  # noqa: C901
        ...
        try:
            ...
            response = ""
            if response_obj and hasattr(response_obj, 'messages') and response_obj.messages:
...
async def run_evaluations_cli(scenario_filter: Optional[str] = None) -> EvaluationSummary:
