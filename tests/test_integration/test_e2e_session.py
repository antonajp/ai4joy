...
class TestE2ESession:
    ...
    @pytest.mark.slow
    def test_complete_session_flow(self, session_client, test_session_config, test_user_inputs):
        """
        ...
        assert 'welcome' in mc_response['message'].lower() or 'hello' in mc_response['message'].lower()
        assert 'location' in mc_response['message'].lower() or 'suggestion' in mc_response['message'].lower()
        ...
        for turn_num, user_input in enumerate(test_user_inputs, start=1):
            ...
            if turn_num <= 4:
                ...
                assert 'yes' in turn_result['partner_response'].lower() or \
                       'and' in turn_result['partner_response'].lower()
            else:
    ...
    def test_room_vibe_check_present(self, session_client, test_session_config):
        ...
        for turn in range(1, 4):
            ...
            assert result['room_vibe']['temperature'] in [
                'Engaged', 'Enthusiastic', 'Neutral', 'Bored', 'Confused', 'Tense'
            ]
...
class SessionAPIClient:
    ...
    def get_mc_intro(self, session_id: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.base_url}/session/{session_id}/mc",
            timeout=10
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_coach_feedback(self, session_id: str) -> Dict:
        ...
        response = self.session.get(
            f"{self.base_url}/session/{session_id}/coach",
            timeout=15
        )
        response.raise_for_status()
        return response.json()
    ...
    def close_session(self, session_id: str) -> Dict:
        ...
        response = self.session.post(
            f"{self.base_url}/session/{session_id}/close",
            timeout=10
        )
        response.raise_for_status()
        return response.json()
